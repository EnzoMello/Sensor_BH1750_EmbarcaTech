...
// Variáveis estáticas para armazenar a configuração do PWM.
static uint slice_num;
static uint channel_num;

/**
 * @brief Converte um ângulo (0-180) para a largura de pulso PWM correspondente em microssegundos.
...
static uint32_t angle_to_pulse_width(uint angle) {
    // Mapeia linearmente a faixa de ângulo [0, 180] para a faixa de pulso [MIN, MAX].
...
void servo_init() {
    ...
    // Associa o pino GPIO à função de hardware PWM.
    gpio_set_function(SERVO_PIN, GPIO_FUNC_PWM);

    // Descobre qual "fatia" (slice) e "canal" de PWM o pino GPIO usa.
    ...
    pwm_config_set_wrap(&config, PWM_PERIOD_MS * 1000);
...
void servo_set_angle(uint angle) {
    ...
    // Converte o ângulo para a largura de pulso e define o nível do canal PWM.
